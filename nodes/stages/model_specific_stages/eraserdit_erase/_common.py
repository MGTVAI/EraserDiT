...
Numerical helpers are ports of the frozen baseline at commit ``9944867``
(``pipelines/pipeline_ltx_video2video.py``); the randomness contract they encode
is documented in ``vibe/plan.md`` §4.8.
"""
...
class EraserDiTTaskState:
    ...
    role of ``pre_video_shift``.  Both are per-request, satisfying the isolation
    requirement in ``vibe/plan.md`` M1.
    """
